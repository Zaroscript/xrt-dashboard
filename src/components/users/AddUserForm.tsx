...
import { CalendarIcon, Plus, X, Globe, MapPin, Building, User, Mail, Phone, CreditCard, Calendar, Info } from 'lucide-react';
...
const formSchema = z.object({
  ...
  status: z.enum(['active', 'inactive', 'pending'] as const).default('active'),
  accountType: z.enum(['admin', 'manager', 'staff'] as const).default('staff'),
  ...
  subscription: z.object({
    ...
    billingCycle: z.enum(['monthly', 'quarterly', 'annually'] as const).default('monthly'),
    startDate: z.date().default(() => new Date()),
    paymentMethod: z.enum(['credit_card', 'bank_transfer', 'paypal'] as const).default('credit_card'),
  }),
  ...
  // System Fields
  isClient: z.literal(true).default(true),
  ...
  message: "All billing address fields are required when different from physical address",
  path: ["billingAddress"],
});
...
export const AddClientForm = ({ onSuccess, onCancel, onFormStateChange }: AddClientFormProps & { onFormStateChange?: (state: any) => void }) => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const clientData = {
        ...
        companyName: values.businessName || `${values.firstName} ${values.lastName}`,
        address: {
      ...
      let errorMessage = 'There was an error creating the client. Please try again.';
