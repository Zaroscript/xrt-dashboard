    ...
    try {
      ...
      if (state) {
        console.log('Auth state:', {
          ...
          accessToken: state.tokens?.accessToken ? '***' + state.tokens.accessToken.slice(-10) : null,
          refreshToken: state.tokens?.refreshToken ? '***' + state.tokens.refreshToken.slice(-10) : null,
        });
