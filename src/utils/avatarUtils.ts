 ...
 */
export const getAvatarUrl = (avatarPath?: string | null, cacheBust?: number): string | undefined => {
  if (!avatarPath) return undefined;
  
  // If they gave us a full URL already, just use it (add cache bust if needed)
  if (avatarPath.startsWith('http://') || avatarPath.startsWith('https://')) {
    return cacheBust ? `${avatarPath}?t=${cacheBust}` : avatarPath;
  }
 ...
 */
export const getUserInitials = (fName?: string, lName?: string, email?: string): string => {
  if (fName && lName) {
